...
Uses OAuth2 authentication for proper Google Drive access
"""
...
CREDENTIALS_FILE = 'oauth_credentials.json'  # You'll need to create this
        ...
        if not creds or not creds.valid:
            ...
            if not creds:
                ...
                try:
                    ...
                    # Check if this is a web or installed app credential
                    with open(CREDENTIALS_FILE, 'r') as f:
                        cred_data = json.load(f)
                    
                    if 'web' in cred_data:
                        # For web credentials, use localhost callback for desktop app
                        flow.redirect_uri = 'http://localhost:8080/callback'
                        print("[INFO] Using web application OAuth2 credentials (localhost callback)")
                    else:
                        # Default for installed app
                        flow.redirect_uri = 'http://localhost:8080/callback'
                        print("[INFO] Using installed application OAuth2 credentials")
                    
                    # Generate authorization URL
                    auth_url, _ = flow.authorization_url(prompt='consent')
                    
                    print("[OAUTH] Please visit this URL to authorize the application:")
                    print(f"[URL] {auth_url}")
                    print("\n[WAITING] After authorization, you'll be redirected to a callback URL.")
                    print("[COPY] Please copy the entire callback URL and paste it here:")
...
Generated by Content Factory Pro OAuth2 Upload System
...
def main():
    ...
    print("\n=== Content Factory Pro OAuth2 Upload ===")
